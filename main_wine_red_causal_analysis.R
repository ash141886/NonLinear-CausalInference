...
#
# Full hyperparameter tuning and performance comparison on red wine data,
# aligned with the new project style but preserving legacy behavior/results.
# ===================================================================
...
# =============================================================================
# SECTION 1: KERNEL-BASED INDEPENDENCE TESTING (LEGACY HSIC FOR REPRODUCIBILITY)
# =============================================================================
...
# HSIC (legacy variant used in your older wine code):
# - bandwidth via median heuristic on joint distances of cbind(x, y)
# - normalization by n^2 (not (n-1)^2)
hsic_legacy <- function(x, y, sigma = NULL) {
  ...
  # legacy normalization to match previous results
  sum(H %*% Kx %*% H * Ky) / n^2
}

# =============================================================================
# SECTION 2: PROPOSED METHOD (LEGACY WINE BEHAVIOR)
# =============================================================================
# Fit GAM for each variable using ALL other variables as smooth/linear terms,
# take residuals, then build a symmetric HSIC matrix on residual pairs.
# Undirected adjacency = HSIC > quantile(threshold) on lower triangle,
# mirrored to keep symmetry. No acyclicity enforcement (same as before).

run_proposed_method_wine <- function(data, sigma = 1, threshold_percentile = 0.8, gam_k_max = 10) {
  n_vars <- ncol(data)
  var_names <- colnames(data)

  # 1) residuals via GAMs with dynamic k
  residuals_list <- vector("list", n_vars)
  for (i in seq_len(n_vars)) {
    target <- var_names[i]
    preds <- setdiff(var_names, target)

    # build terms with dynamic k
    terms <- vapply(preds, function(pn) {
      n_unique <- length(unique(data[[pn]]))
      k_dynamic <- min(gam_k_max, n_unique - 1)
      if (!is.finite(k_dynamic) || k_dynamic < 3) {
        paste0("`", pn, "`")
      } else {
        paste0("s(`", pn, "`, k=", k_dynamic, ")")
      }
    }, FUN.VALUE = character(1))

    fml <- as.formula(paste0("`", target, "` ~ ", paste(terms, collapse = " + ")))
    gm <- tryCatch(
      gam(fml, data = data, method = "REML"),
      error = function(e) NULL
    )
    residuals_list[[i]] <- if (!is.null(gm)) residuals(gm) else rnorm(nrow(data))
  }

  # 2) symmetric HSIC matrix on residual pairs (legacy hsic)
  hsic_matrix <- matrix(0, n_vars, n_vars, dimnames = list(var_names, var_names))
  for (i in 1:(n_vars - 1)) {
    for (j in (i + 1):n_vars) {
      val <- tryCatch(hsic_legacy(residuals_list[[i]], residuals_list[[j]], sigma),
                      error = function(e) 0)
      hsic_matrix[i, j] <- val
      hsic_matrix[j, i] <- val
    }
  ...
  # 3) threshold on lower triangle to build undirected adjacency
  lower_vals <- hsic_matrix[lower.tri(hsic_matrix)]
  threshold <- as.numeric(quantile(lower_vals, threshold_percentile, na.rm = TRUE))
  estimated_adj_matrix <- hsic_matrix > threshold

  # force symmetry and clear diagonal
  estimated_adj_matrix <- estimated_adj_matrix | t(estimated_adj_matrix)
  diag(estimated_adj_matrix) <- 0

  estimated_adj_matrix
}

# =============================================================================
# SECTION 3: BASELINE LiNGAM (LEGACY WINE BEHAVIOR)
# =============================================================================
...
# =============================================================================
# SECTION 4: GROUND TRUTH & EVALUATION (UNDIRECTED, LEGACY)
# =============================================================================
...
# Undirected metrics (precision/recall/F1 etc.) exactly as before
...
# =============================================================================
# SECTION 5: FULL HYPERPARAMETER TUNING (UNCHANGED GRID & LOGIC)
# =============================================================================
